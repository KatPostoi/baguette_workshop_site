...
export const AdminDataPage = () => {
  ...
  const handleServiceSave = async () => {
    try {
      if (serviceDraft.id) {
        const updated = await adminUpdateService(
          Number(serviceDraft.id),
          serviceDraft
        );
        setServices((prev) =>
          prev.map((s) => (s.id === updated.id ? updated : s))
        ...
        const created = await adminCreateService(serviceDraft);
  ...
  return (
    <AdminShell title="Данные" active="data">
      ...
      <AdminSection title="Каталог">
        ...
        <AdminTable headers={['ID', 'Название', 'Цена', 'Остаток', 'Материал', 'Стиль', '']}>
          {!loading && !catalog.length ? <div className="admin-empty">Нет позиций.</div> : null}
          {catalog
            .filter((item) =>
              catalogSearch
                ? `${item.title} ${item.slug} ${item.color}`.toLowerCase().includes(catalogSearch.toLowerCase())
                : true
            )
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setCatalogDraft({
                        ...
                        stock: item.stock,
                        color: item.color,
                        materialId: (item.material as unknown as { id?: number })?.id,
      ...
      <AdminSection title="Материалы">
        ...
        <AdminTable headers={['ID', 'Название', 'Цена за см', '']}>
          {!loading && !materials.length ? <div className="admin-empty">Нет материалов.</div> : null}
          {materials
            .filter((m) =>
              materialSearch ? `${m.title} ${m.material}`.toLowerCase().includes(materialSearch.toLowerCase()) : true
            )
      ...
      <AdminSection title="Стили">
        ...
        <AdminTable headers={['ID', 'Название', 'Коэф.', '']}>
          {!loading && !styles.length ? <div className="admin-empty">Нет стилей.</div> : null}
          {styles
            .filter((s) => (styleSearch ? `${s.id} ${s.name}`.toLowerCase().includes(styleSearch.toLowerCase()) : true))
            ...
            .map((s) => (
              <div key={s.id} className="admin-table__row">
                ...
                <div className="admin-table__actions">
                  <Button variant="secondary" onClick={() => setStyleDraft(s)}>
                    Редактировать
                  </Button>
                  <Button
                    variant="secondary"
      ...
      <AdminSection title="Услуги">
        ...
        <AdminTable headers={['ID', 'Тип', 'Название', 'Цена', '']}>
          {!loading && !services.length ? <div className="admin-empty">Нет услуг.</div> : null}
          {services
            .filter((s) =>
              serviceSearch ? `${s.title} ${s.type}`.toLowerCase().includes(serviceSearch.toLowerCase()) : true
            )
            ...
            .map((s) => (
              <div key={s.id} className="admin-table__row">
                ...
                <div>{s.price}</div>
                <div className="admin-table__actions">
                  <Button variant="secondary" onClick={() => setServiceDraft(s)}>
                    Редактировать
                  </Button>
                  <Button
                    variant="secondary"
      ...
      <AdminSection title="Пользователи">
        ...
        <AdminTable headers={['ID', 'Email', 'Имя', 'Телефон', 'Роль']}>
          {!loading && !users.length ? <div className="admin-empty">Нет пользователей по фильтру.</div> : null}
          {users
